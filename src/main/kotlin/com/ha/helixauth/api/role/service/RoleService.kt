...
class RoleService(@Autowired private val roleRepository: RoleRepository,
    ...
    fun createRole(name: String, companyId: Long, smartContract: String): Role {
        val company = companyRepository.findById(companyId)
            .orElseThrow { RuntimeException("Company not found") }
        val role = Role(name = name, smartContract = smartContract, company = company)
        roleRepository.save(role)

        // Add a blockchain transaction
        val roleCreationData = "Created Role: $name, Contract: $smartContract"
