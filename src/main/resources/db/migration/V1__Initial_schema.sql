CREATE TABLE IF NOT EXISTS company (
    ...
    name VARCHAR(255) UNIQUE, -- UNIQUE constraint on 'name'
    phone VARCHAR(255),
    street VARCHAR(255),
    city VARCHAR(255),
    state VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS api_key (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY,
    key VARCHAR(255) NOT NULL UNIQUE, -- UNIQUE constraint on 'key'
    company_id BIGINT,
    PRIMARY KEY (id),
    FOREIGN KEY (company_id) REFERENCES company (id)
);
...
CREATE TABLE IF NOT EXISTS endpoint_config (
    ...
    PRIMARY KEY (id),
    FOREIGN KEY (company_id) REFERENCES company (id),
    UNIQUE (path, operation, company_id) -- UNIQUE constraint on combination
);
